...
function(input, output) {
  sliderValues <- reactive({
    if(input$margin == "add"){
      ...
      Values =  as.numeric(add.margin(delta = input$marginval, varC = input$varC, varT = input$varT,
      ...
      data.frame(Output = c("Sample Size in Control",
                            ...
                            "Randomization treatment to control (k:1)",
                            "Realtive Efficiency (compared to 1:1)"),
                 Values = as.numeric(multi.margin(delta = input$marginval, muC = input$muC, varC = input$varC, varT = input$varT,
                 alpha = as.numeric(input$type1), beta = 1 - as.numeric(input$power))))
    }
  })
  eff <- reactive({
      if(input$margin == "add"){
          data.frame(control = h * sum(as.numeric(add.margin(delta = input$marginval, varC = input$varC, varT = input$varT,
          ...
          data.frame(control = h * sum(as.numeric(multi.margin(delta = input$marginval, muC = input$muC, varC = input$varC, varT = input$varT,
                                                               alpha = as.numeric(input$type1), beta = 1 - as.numeric(input$power)))[1:2]),
